// 这是一个简化版的 std::function 实现，支持 C++17 标准
// 核心设计：类型擦除 (Type Erasure)
...
struct Function
{
    ...
    // static_assert 编译阶段检查条件是否成立
    // 触发这个静态断言，导致编译失败，并显示错误信息 "not a valid function signature"
    static_assert(!std::is_same_v<_FnSig, _FnSig>, "not a valid function signature");
};

// 声明了两个class占位符；class...表示可代表多个值
template <class _Ret, class... _Args>
struct Function<_Ret(_Args...)> 
// 结构体模板特化，针对函数类型进行实现
// <_Ret(_Args...)>表示一个函数类型，_Ret是返回类型，_Args...是参数类型列表
{
private:
    struct _FuncBase 
    // 做了一个基类，所有的仿函数类都继承自这个基类
    {
    ...
    // template<class... Args>  // ... 标记 class：表示"这是一包类型"
    // 这里 ... 是给 class 用的，告诉编译器 Args 可以代表多个类型
    // void func(Args... args)  // ... 标记 args：表示"这是一个包变量"
    // 这里 ... 是给 args 用的，告诉编译器 args 可以接收多个值
    // func(args...);  // ... 标记 args：表示"展开这个包"
    // 这里 ... 是给 args 用的，把包里的多个值展开
...
public:
    ...
    template <class _Fn, class = std::enable_if_t<std::is_invocable_r_v<_Ret, std::decay_t<_Fn>, _Args...> && std::is_copy_constructible_v<_Fn> && !std::is_same_v<std::decay_t<_Fn>, Function<_Ret(_Args...)>>>>
